...
import time
from ServidorFake import ServidorFake
        ...
        try:
            ...
            try:
                ...
                if not os.path.exists(archivo_cache) or os.path.getsize(archivo_cache) == 0:
                    print(f"Advertencia: No se pudo guardar correctamente el embedding en {archivo_cache}")
            except Exception as e:
...
class LSTMRegressor(nn.Module):
    def __init__(self, embedding_dim, dim_oculta, tasa_dropout=0.5): 
        ...
        self.entrada_dropout = LockedDropout(0.2)
...
def main():
    ...
    if confirmacion.lower() in ['si', 'y', 's']:
        ...
        for i in range(3):
            ...
            multi_counts = {'Dataset': ['Train', 'Dev', 'Test', 'Total']}
            
            for label in multi_labels:
                multi_counts[f'Clase {label}'] = [
            ...
            multi_counts['Total'] = [len(multi_train), len(multi_dev), len(multi_test), len(multi_etiquetas)]
            
            print(pd.DataFrame(multi_counts).to_string(index=False))
            ...
            if set(dict_gold_test.keys()) != set(dict_gold_test_multi.keys()):
                raise Exception("Error: Las IDs entre el gold binario y multiclass no coinciden.")
            ...
            print("Esperando 1 segundo antes de continuar...")
            time.sleep(1)
...
def entrenar_y_guardar_modelo(ruta_guardado="./modelos_entrenados_finales"):
    ...
    print("\nbinarias Classification Distribution:")
    print(pd.DataFrame(porcentaje_tarea_binaria).to_string(index=False))
    
    multi_labels = sorted(np.unique(multi_etiquetas))
    multi_counts = {'Dataset': ['Train', 'Dev', 'Total']}
    ...
    multi_counts['Total'] = [len(multi_train), len(multi_dev), len(multi_etiquetas)]
